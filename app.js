...
document.addEventListener('DOMContentLoaded', () => {
                ...
                <div class="subscription-grid">
                    ${subscriptions.length > 0 ? 
                        subscriptions.map(sub => `
                            <div class="subscription-card" data-link="${sub.link}" data-title="${sub.title}" data-thumbnail="${sub.thumbnail}">
            ...
            <div class="account-section">
                ${currentUser ? `
                    <div class="user-profile">
                        <img src="${currentUser.photoURL}" alt="Profile" class="profile-picture">
                        <div class="user-info">
                ...
                <div class="detail-header">
                    ...
                    <div class="detail-info">
                        ...
                        <button id="subscribe-btn" class="subscribe-btn ${isSubscribed ? 'subscribed' : ''}" 
                                data-title="${title}" data-thumbnail="${thumbnail}" data-link="${data.episodes?.[0]?.link || ''}">
    ...
    const handleSearch = async (query) => {
        ...
        try {
            ...
            resultsContainer.innerHTML = '<p class="error-message">Gagal mencari anime.</p>';
        }
    };
    ...
    const toggleSubscription = (title, thumbnail, link) => {
        ...
        if (existingIndex > -1) {
            ...
            subscriptions.push({ title, thumbnail, link });
        }
    ...
    app.addEventListener('click', e => {
        ...
        const card = e.target.closest('.anime-card, .search-result-card, .slide-card, .subscription-card');
        if (card) {
            e.preventDefault();
        ...
        if (e.target.closest('#subscribe-btn')) {
            ...
            const isNowSubscribed = toggleSubscription(title, thumbnail, link);
